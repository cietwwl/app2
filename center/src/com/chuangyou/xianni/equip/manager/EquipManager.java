...
public class EquipManager {
	...
	public static void weaponAwaken(GamePlayer player, ItemPosMsg itemPos, short protocol) {
		...
		if (!player.getBagInventory().removeItemFromPlayerBag(cfg.getNeedItem(), cfg.getNeedItemNum(), ItemRemoveType.EQUIP_AWAKEN))
			return;
	...
	public static boolean equipResolve(GamePlayer player, ItemPosMsg req, short protocol, boolean hint) {
		...
		// EquipResolveCfg cfg =
		// EquipTemplateMgr.getResolveMap().get(equip.getItemTempInfo().getId());
		// if(cfg == null){
		// ErrorMsgUtil.sendErrorMsg(player, ErrorCode.EQUIP_UN_RESOLVE,
		// protocol, "此装备不可分解");
		// return;
		// }
		...
		// player.getBagInventory().addItemInBagOrEmail(key, items.get(key),
		// ItemAddType.EQUIP_RESOLVE, true);
		// }
		// 经验
		// int exp = equip.getItemTempInfo().getExp();
		// player.getBasePlayer().addEquipExp(exp);
		int count = equip.getItemTempInfo().getResolveCount();
		if (equip.getItemTempInfo().getResolveId() == CurrencyItemType.EQUIP_EXP) {
			MagicwpBanInfo magInfo = player.getMagicwpInventory().getBan(MagicwpBanConstant.DECOMPOSE.getCode());
			if (magInfo != null && magInfo.getPosition() > 0) {
				count += count * Math.min(5 + (magInfo.getLevel() - 1) * 3, 50) / 100;
			}
		}
		player.getBagInventory().addItemInBagOrEmail(equip.getItemTempInfo().getResolveId(), count, ItemAddType.EQUIP_RESOLVE, true);
