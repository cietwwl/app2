...
public class SkillTempMgr {
	 ...
	 * 技能属性配置
	 */
	private static Map<Integer, SkillPropertyTemplateInfo> skillProTemp = new HashMap<Integer, SkillPropertyTemplateInfo>();
	/**
	 * 技能阶段
	 */
	...
	public static boolean reloadPb() {
		...
		if (skillTempInfos != null && skillTempInfos.size() > 0) {
			for (SkillTempateInfo stemp : skillTempInfos) {
				// 加入属性
				SkillPropertyTemplateInfo proTemp = parsePro(stemp.getPropertyIds());
				stemp.setProTemp(proTemp);
				skillTemp.put(stemp.getTemplateId(), stemp);
			}
		}
		// 加载技能属性模板
		List<SkillPropertyTemplateInfo> skillProTempList = DBManager.getSkillPropertyTemplateInfoDao().load();
		if (skillProTempList != null && !skillProTempList.isEmpty()) {
			for (SkillPropertyTemplateInfo skillPropertyTemplateInfo : skillProTempList) {
				skillProTemp.put(skillPropertyTemplateInfo.getTemplateId(), skillPropertyTemplateInfo);
			}
		}
		// 技能阶段
...
//		Map<Integer, SkillTempateInfo> skillTempMap = new HashMap<Integer, SkillTempateInfo>();
	...
	/** 解析属性 */
	private static SkillPropertyTemplateInfo parsePro(String str) {
		SkillPropertyTemplateInfo temp = new SkillPropertyTemplateInfo();
		int[] pro = SplitUtil.splitToInt(str);
		for (int i : pro) {
			int key = i / 1000000;
			int val = i % 1000000;
			assignPro(temp, key, val);
		}
		return temp;
	}

	/** 属性赋值 */
	public static SkillPropertyTemplateInfo assignPro(SkillPropertyTemplateInfo temp, int key, int val) {
		switch (key - 1) {
		case Living.SOUL:
			temp.setSoul(val);
			break;
