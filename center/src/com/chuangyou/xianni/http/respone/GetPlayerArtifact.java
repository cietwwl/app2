...
import com.chuangyou.xianni.http.util.CalArtifact;
import com.chuangyou.xianni.player.BasePlayer;
import com.chuangyou.xianni.player.GamePlayer;
import com.chuangyou.xianni.sql.dao.DBManager;
import com.chuangyou.xianni.word.WorldMgr;

@HttpCmd(command = "getPlayerArtifa", desc = "查询玩家神器")
public class GetPlayerArtifact implements BaseRespone {
		...
		if (nickName != null && !nickName.equals("")) {
			...
			if (info != null) {
				...
				if (player != null && player.getPlayerState() == PlayerState.ONLINE) {
					data = CalArtifact.computeArtifactPor(player);
				} else {
					...
					ArtifactInventory artifactInventory = new ArtifactInventory(player);
					player.setArtifactInventory(artifactInventory);
					artifactInventory.loadFromDataBase();
