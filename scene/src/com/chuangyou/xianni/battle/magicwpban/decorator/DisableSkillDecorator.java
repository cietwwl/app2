...
public class DisableSkillDecorator extends MagicwpBanBaseDecorator {
	private static final int	BUFFER_ID	= 70002;
	private int					fixTime;

	public DisableSkillDecorator(Player master, int level) {
		super(master, level, MagicwpBanConstant.DISABLE_SKILL);
		fixTime = Math.min(1000 + (level - 1) * 200, 4000);
	}
