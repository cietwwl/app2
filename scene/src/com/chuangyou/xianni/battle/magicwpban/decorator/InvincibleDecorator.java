...
public class InvincibleDecorator extends MagicwpBanBaseDecorator {
	static final int	BUFFER_ID	= 70004;
	private int			fixTime;

	public InvincibleDecorator(Player master, int level) {
		super(master, level, MagicwpBanConstant.INVINCIBLE);
	...
	public void exe() {
		...
		Buffer buffer = BufferFactory.createBuffer(master, master, temp);
		buffer.setAliveTime(System.currentTimeMillis() + fixTime);
		master.addBuffer(buffer);
