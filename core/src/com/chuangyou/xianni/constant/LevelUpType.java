package com.chuangyou.xianni.constant;

public class LevelUpType {

	/**
	 * 玩家角色升级
	 */
	public static final int PLAYER = 1;
	
	/**
	 * 帮派升级
	 */
	public static final int GUILD = 2;
	
	/**
	 * 帮派主殿升级
	 */
	public static final int GUILD_MAINBUILD = 3;
	
	/**
	 * 帮派蒧经阁升级
	 */
	public static final int GUILD_SKILLSHOP = 4;
	
	/**
	 * 帮派商店升级
	 */
	public static final int GUILD_SHOP = 5;
	
	/**
	 * 帮派仓库升级
	 */
	public static final int GUILD_WAREHOUSE = 6;
	
	
	/**
	 * 装备栏升级类型为栏位位置加上该起始值
	 * 10-17装备栏位升级占用
	 */
	public static final int EQUIPBARSTART = 10;
	
	/**
	 * 魂幡
	 */
	public static final int SOUL = 21;
}
