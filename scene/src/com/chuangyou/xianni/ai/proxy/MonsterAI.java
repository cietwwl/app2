...
public class MonsterAI extends BaseProxy {
	...
	protected void exe() {
		...
		// return;
		// }
		if (!living.checkStatus(LivingState.MOVE)) {
			((Monster) living).stop(false);
			return;
		}

		AIState next = behaviors.get(current).next();
//		if (living.getId() == 1000000000035L){
//			System.out.println("怪物id： " + living.getId() + " 状态： " + current + " 下一个状态：" + next+" 位置："+living.getPostion()+" 目标："+living.getTargetPostion());
//		}
