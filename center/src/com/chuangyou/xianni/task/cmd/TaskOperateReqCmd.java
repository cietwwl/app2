...
public class TaskOperateReqCmd extends AbstractCommand {
	...
	public void execute(GamePlayer player, PBMessage packet) throws Exception {
		...
		if(operate == 1){
			...
			if(cfg.getMoney()!=0){
				player.getBasePlayer().addMoney((int)cfg.getMoney());
			}
			if(cfg.getBindCash()!=0){
				player.getBasePlayer().addBindCash(cfg.getBindCash());
			}
			if(!StringUtils.isNullOrEmpty(cfg.getItems())){
				...
				while(it.hasNext()){
					...
					player.getBagInventory().addItemInBagOrEmail(Integer.parseInt(s[0]),
							Integer.parseInt(s[1]), ItemAddType.TASK_ADD,Boolean.parseBoolean(s[2]));
				}
			}
