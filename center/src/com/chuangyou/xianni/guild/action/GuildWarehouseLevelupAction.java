...
public class GuildWarehouseLevelupAction extends GuildIsGuildMemberAction {

	public GuildWarehouseLevelupAction(GamePlayer player, PBMessage packet) {
		super(player, packet);
	...
	public void execute(GamePlayer player, PBMessage packet, Guild guild, GuildMemberInfo memberInfo) {
		...
		int maxLevel = LevelUpTempleteMgr.getMaxLevel(LevelUpType.GUILD_WAREHOUSE);
		if(guild.getGuildInfo().getWarehouseLevel() >= maxLevel){
			ErrorMsgUtil.sendErrorMsg(player, ErrorCode.GUILD_WAREHOUSE_MAXLEVEL, packet.getCode(), "帮派仓库已达到最高等级");
			return;
		}
		
		LevelUp levelUpCfg = LevelUpTempleteMgr.getLevelUp(LevelUpType.GUILD_WAREHOUSE, guild.getGuildInfo().getWarehouseLevel());
		...
		guild.getGuildInfo().setWarehouseLevel(guild.getGuildInfo().getWarehouseLevel() + 1);
		
		GuildRespMsg.Builder respMsg = GuildRespMsg.newBuilder();
		respMsg.setAction(GuildOperateAction.WAREHOUSE_LEVELUP);
		respMsg.setParam1(guild.getGuildInfo().getWarehouseLevel());
