...
public class C3p0Pool implements IDBPool {
	 ...
	 *            此连接池允许建立的最大连接数
	 */
	public C3p0Pool(String dbConnUrl, String dbUserName, String dbPassWord, int maxConn, int fallow) {
		try {
			...
			dataSource.setInitialPoolSize(fallow); // 初始化
			dataSource.setMinPoolSize(fallow); // 连接池中保留的最小连接数
			dataSource.setMaxPoolSize(maxConn); // 连接池中保留的最大连接数
			...
			Log.error("Init CPool has exception:", e);
		}
	}
	...
	public void shutdown() {
		try {
			...
			Log.error("dataSource close Exception ", e);
		}
	}
