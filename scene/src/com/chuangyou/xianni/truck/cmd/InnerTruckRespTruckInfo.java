...
public class InnerTruckRespTruckInfo extends AbstractCommand {
	...
	public void execute(ArmyProxy army, PBMessage packet) throws Exception {
		...
		// {
		// CheckPointRespHelper.respCheckPointStatus(army,
		// CheckPointRespHelper.CHECKPOINT_CANTCREATE, 0);
		// return;
		// }
		Player owner = WorldMgr.getArmy(army.getPlayerId()).getPlayer();
		Map<Integer, List<TruckSkillInfo>> skills = getSkillInfos(truckInfo.getTruckData().getSkillsList(), truckInfo.getTruckerData().getSkillsList(), truckInfo.getTruckerData().getGiftsList());
