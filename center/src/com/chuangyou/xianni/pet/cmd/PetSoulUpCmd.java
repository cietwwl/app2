...
public class PetSoulUpCmd extends AbstractCommand {
	...
	public void execute(GamePlayer player, PBMessage packet) throws Exception {
		...
		if (needNum > 0) {
			...
			if (!player.getBagInventory().removeItemFromPlayerBag(petSoul.getNeedItem(), needNum, ItemRemoveType.PET_SOUL_UP))
				return;
		}
		if (moneyNum > 0) {
			...
			if (!player.getBasePlayer().consumeCash(moneyNum, ItemRemoveType.PET_SOUL_UP))
				return;
		}
		// 回复
