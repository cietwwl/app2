...
public class CardSkillLogic extends AbstractCardLogic {
	...
	public void doCardLogic() {
		...
		int costNum = SoulTemplateMgr.getCardSkillCofig(getOldSkillID()).getCostNum();
		
		int count = this.cardInfo.getRemainTime()+(int)(player.getBagInventory().getItemCount(BagType.VirtualValue,this.cardId)/costNum);;
		if(count==0){
		...
		if(this.cardInfo.getRemainTime()>0){			
			...
			if(!player.getBagInventory().removeItem(BagType.VirtualValue,this.cardId, costNum, ItemRemoveType.SOUL)){
